...
export default function Header() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled
        ...
        <div className="flex justify-between items-center py-4">
          ...
          <Link href="/" className="group">
            <div className="text-2xl font-bold text-black transform transition-all duration-300 group-hover:scale-110">
              <span className="text-5xl group-hover:text-gray-700">
                1vans
              </span>
            </div>
          </Link>
          ...
          >
            <i className={`${isMenuOpen ? 'ri-close-line' : 'ri-menu-line'} text-xl text-black transition-transform duration-300 ${isMenuOpen ? 'rotate-180' : ''
              }`}></i>
          </button>
        </div>
      </div>
      ...
      {/* Mobile Menu */}
      <div className={`md:hidden backdrop-blur-md bg-white/90 border-b border-white/20 transition-all duration-500 ${isMenuOpen
